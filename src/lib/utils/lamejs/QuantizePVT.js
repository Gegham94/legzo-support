...
function QuantizePVT() {
   ...
   */
  const IXMAX_VAL = QuantizePVT.IXMAX_VAL;

  const PRECALC_SIZE = IXMAX_VAL + 2;

  const Q_MAX = QuantizePVT.Q_MAX;
   ...
   * </CODE>
   */
  const Q_MAX2 = QuantizePVT.Q_MAX2;

  const LARGE_BITS = QuantizePVT.LARGE_BITS;
  ...
  const pow43 = new_float(PRECALC_SIZE);

  const adj43 = new_float(PRECALC_SIZE);
  this.adj43 = adj43;
  ...
  this.on_pe = function (gfp, pe, targ_bits, mean_bits, gr, cbr) {
    ...
    let tbits = 0;
    let bits;
    const add_bits = new_int(2);
    let ch;

    /* allocate targ_bits for granule */
    const mb = new MeanBits(tbits);
    let extra_bits = rv.ResvMaxBits(gfp, mean_bits, mb, cbr);
    tbits = mb.bits;
  ...
  this.athAdjust = function (a, x, athFloor) {
    ...
    const o = 90.30873362;
    const p = 94.82444863;
    let u = Util.FAST_LOG10_X(x, 10.0);
    const v = a * a;
    let w = 0.0;
    u -= athFloor;
    /* undo scaling */
  ...
  this.set_pinfo = function (gfp, cod_info, ratio, gr, ch) {
    ...
    const xfsf = new_float(L3Side.SFBMAX);
