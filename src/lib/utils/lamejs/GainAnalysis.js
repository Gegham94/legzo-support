...
function GainAnalysis() {
   ...
   */
  const PINK_REF = 64.82;

  const YULE_ORDER = GainAnalysis.YULE_ORDER;
  /**
   * percentile which is louder than the proposed level
   */
  const RMS_PERCENTILE = 0.95;
  /**
   * maximum allowed sample frequency [Hz]
   */
  const MAX_SAMP_FREQ = GainAnalysis.MAX_SAMP_FREQ;
  const RMS_WINDOW_TIME_NUMERATOR = GainAnalysis.RMS_WINDOW_TIME_NUMERATOR;
  /**
   * numerator / denominator = time slice size [s]
   */
  const RMS_WINDOW_TIME_DENOMINATOR = GainAnalysis.RMS_WINDOW_TIME_DENOMINATOR;
  /**
   * max. Samples per Time slice
   */
  const MAX_SAMPLES_PER_WINDOW = GainAnalysis.MAX_SAMPLES_PER_WINDOW;
  ...
  function analyzeResult(Array, len) {
    ...
    let upper = 0 | Math.ceil(elems * (1 - RMS_PERCENTILE));
    for (i = len; i-- > 0; ) {
  ...
  this.GetTitleGain = function (rgData) {
    const retval = analyzeResult(rgData.A, rgData.A.length);
