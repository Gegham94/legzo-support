...
function PsyModel() {
  ...
  function calc_mask_index_l(gfc, max, avg, mask_idx) {
    const last_tab_entry = tab.length - 1;
    let b = 0;
    let a = avg[b] + avg[b + 1];
    assert(a >= 0);
    if (a > 0.0) {
  ...
  this.L3psycho_anal_ns = function (
    ...
    const mask_idx_s = new_int(Encoder.CBANDS + 2);
    ...
    for (chn = 0; chn < gfc.channels_out; chn++) {
      ...
      const firbufPos = bufPos + 576 - 350 - NSFIRLEN + 192;
      ...
      for (i = 0; i < 576; i++) {
        ...
        for (j = 0; j < (NSFIRLEN - 1) / 2 - 1; j += 2) {
          sum1 +=
            fircoef[j] *
            (firbuf[firbufPos + i + j] + firbuf[firbufPos + i + NSFIRLEN - j]);
          sum2 +=
            fircoef[j + 1] *
    ...
    for (chn = 0; chn < numchn; chn++) {
      ...
      k = 0;
      {
        for (b = 0; b < gfc.npart_l; b++) {
          ...
          let eb2 = eb_l[kk] * tab[mask_idx_l[kk]];
  ...
  function vbrpsy_compute_masking_s(gfp, fftenergy_s, eb, thr, chn, sblock) {
    const gfc = gfp.internal_flags;
    const max = new float[Encoder.CBANDS]();
    const avg = new_float(Encoder.CBANDS);
    let i, j, b;
    const mask_idx_s = new int[Encoder.CBANDS]();
  ...
  function vbrpsy_compute_masking_l(gfc, fftenergy, eb_l, thr, chn) {
    ...
    for (b = 0; b < gfc.npart_l; b++) {
      ...
      if (gfc.blocktype_old[chn & 0x01] == Encoder.SHORT_TYPE) {
        ...
        let ecb_limit_2 = rpelev2 * gfc.nb_2[chn][b];
        let ecb_limit_1 = rpelev * gfc.nb_1[chn][b];
        var ecb_limit;
        if (ecb_limit_2 <= 0) {
          ecb_limit_2 = ecb;
        }
  ...
  function init_s3_values(s3ind, npart, bval, bval_width, norm, use_old_s3) {
    ...
    if (use_old_s3) {
      ...
      for (j = 0; j < npart; j++) {
        ...
        const s3_x_norm = norm_s3_func_x(hf_slope);
        for (var i = 0; i < npart; i++) {
          var v =
            s3_x_norm * s3_func_x(bval[i] - bval[j], hf_slope) * bval_width[j];
          s3[i][j] = v * norm[i];
        }
      }
    }
    ...
    const p = new_float(numberOfNoneZero);
    let k = 0;
    for (var i = 0; i < npart; i++) {
  ...
  this.psymodel_init = function (gfp) {
    ...
    let snr_s_a = -8.25;
    let snr_s_b = -4.5;
    const bval = new_float(Encoder.CBANDS);
    const bval_width = new_float(Encoder.CBANDS);
    const norm = new_float(Encoder.CBANDS);
