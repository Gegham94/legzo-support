...
function Quantize() {
  ...
  function psfb21_analogsilence(gfc, cod_info) {
    ...
    if (cod_info.block_type != Encoder.SHORT_TYPE) {
      ...
      for (var gsfb = Encoder.PSFB21 - 1; gsfb >= 0 && !stop; gsfb--) {
        ...
        let ath21 = qupvt.athAdjust(ath.adjust, ath.psfb21[gsfb], ath.floor);
      ...
      for (let block = 0; block < 3; block++) {
        ...
        for (var gsfb = Encoder.PSFB12 - 1; gsfb >= 0 && !stop; gsfb--) {
          ...
          let ath12 = qupvt.athAdjust(ath.adjust, ath.psfb12[gsfb], ath.floor);
  ...
  this.trancate_smallspectrums = function (gfc, gi, l3_xmin, work) {
    const distort = new_float(L3Side.SFBMAX);

    if (
      ((gfc.substep_shaping & 4) == 0 && gi.block_type == Encoder.SHORT_TYPE) ||
  ...
  function loop_break(cod_info) {
    for (let sfb = 0; sfb < cod_info.sfbmax; sfb++) {
      if (
        cod_info.scalefac[sfb] + cod_info.subblock_gain[cod_info.window[sfb]] ==
        0
      ) {
        return false;
      }
  ...
  function balance_noise(gfp, cod_info, distort, xrpow, bRefine) {
    ...
    let status = loop_break(cod_info);
  ...
  this.outer_loop = function (gfp, cod_info, l3_xmin, xrpow, ch, targ_bits) {
    ...
    const save_xrpow = new_float(576);
    const distort = new_float(L3Side.SFBMAX);
  ...
  this.get_framebits = function (gfp, frameBits) {
    ...
    let bitsPerFrame = bs.getframebits(gfp);
  ...
  this.VBR_old_prepare = function (
    ...
    const avg = rv.ResvFrameBegin(gfp, new MeanBits(0)) / gfc.mode_gr;

    get_framebits(gfp, frameBits);

    for (var gr = 0; gr < gfc.mode_gr; gr++) {
      const mxb = qupvt.on_pe(gfp, pe, max_bits[gr], avg, gr, 0);
  ...
  this.VBR_new_prepare = function (
    ...
    for (var gr = 0; gr < gfc.mode_gr; gr++) {
      ...
      for (var ch = 0; ch < gfc.channels_out; ++ch) {
        ...
        init_outer_loop(gfc, cod_info);
        if (
          qupvt.calc_xmin(gfp, ratio[gr][ch], cod_info, l3_xmin[gr][ch]) != 0
        ) {
          analog_silence = 0;
        }
