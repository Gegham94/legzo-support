...
function BitStream() {
  ...
  function drain_into_ancillary(gfp, remainingBits) {
    ...
    if (remainingBits >= 32) {
      const version = ver.getLameShortVersion();
      if (remainingBits >= 32) {
  ...
  function ShortHuffmancodebits(gfc, gi) {
    ...
    let bits = Huffmancode(gfc, gi.table_select[0], 0, region1Start, gi);
    bits += Huffmancode(
      gfc,
      gi.table_select[1],
      region1Start,
  ...
  function compute_flushbits(gfp, total_bytes_output) {
    const gfc = gfp.internal_flags;
    let flushbits, remaining_headers;
    let bitsPerFrame;
    let last_ptr, first_ptr;
    first_ptr = gfc.w_ptr;
  ...
  this.flush_bitstream = function (gfp) {
    ...
    if (gfc.findReplayGain) {
      const RadioGain = ga.GetTitleGain(gfc.rgdata);
