...
function VBRTag() {
  ...
  const NUMTOCENTRIES = VBRTag.NUMTOCENTRIES;

  /**
   * (0xB40) the max freeformat 640 32kHz framesize.
   */
  const MAXFRAMESIZE = VBRTag.MAXFRAMESIZE;
   ...
   * </PRE>
   */
  const VBRHEADERSIZE = NUMTOCENTRIES + 4 + 4 + 4 + 4 + 4;

  const LAMEHEADERSIZE =
   ...
   */
  const ISO_8859_1 = null; // Charset.forName("ISO-8859-1");
  ...
  this.getVbrTag = function (buf) {
    ...
    /* get flags */
    const head_flags = (pTagData.flags = extractInteger(buf, bufPos));
    bufPos += 4;
  ...
  this.InitVbrTag = function (gfp) {
    ...
    const headerSize = gfc.sideinfo_len + LAMEHEADERSIZE;
    ...
    const buffer = new_byte(MAXFRAMESIZE);
  ...
  function putLameVBR(gfp, musicLength, streamBuffer, streamBufferPos, crc) {
    ...
    const version = v.getLameVeryShortVersion();
    let vbr;
    const revision = 0x00;
    let revMethod;
  ...
  this.getLameTagFrame = function (gfp, buffer) {
    ...
    // Create TOC entries
    const toc = new_byte(NUMTOCENTRIES);
  ...
  this.putVbrTag = function (gfp, stream) {
    ...
    // tag data.
    const id3v2TagSize = skipId3v2(stream);

    // Seek to the beginning of the stream
    stream.seek(id3v2TagSize);

    const buffer = new_byte(MAXFRAMESIZE);
    const bytes = getLameTagFrame(gfp, buffer);
