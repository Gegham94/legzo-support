...
function Presets() {
  ...
  function apply_vbr_preset(gfp, a, enforce) {
    const vbr_preset =
      gfp.VBR == VbrMode.vbr_rh ? vbr_old_switch_map : vbr_psy_switch_map;
  ...
  function apply_abr_preset(gfp, preset, enforce) {
    ...
    if (Math.abs(abr_switch_map[r].nsbass) > 0) {
      let k = int(abr_switch_map[r].nsbass * 4);
      if (k < 0) k += 64;
