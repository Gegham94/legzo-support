  ...
  >
    ...
    >
      ...
      <div class="vac-icon-textarea">
        ...
        <div class="vac-private-message">
          <el-switch v-model="privateMessageStatus" active-text="Private" />
        </div>

        <div>
          <hashtag-picker-container
            @insert-text="insertText"
            :hashtagKeyPressed="hashtagKeyPressed"
          />
        </div>
...
import HashtagPickerContainer from "../../components/HashtagPickerContainer/HashtagPickerContainer.vue";
...
export default {
  ...
  methods: {
    ...
    insertText(text) {
      const startPos = this.getTextareaRef().selectionStart;
      const endPos = this.getTextareaRef().selectionEnd;
      const currentValue = this.getTextareaRef().value;
      const newText =
        currentValue.substring(0, startPos - 1) +
        text +
        currentValue.substring(endPos);

      this.hashtagKeyPressed = false;
      this.getTextareaRef().value = newText;
    ...
    onChangeInput(event) {
      ...
      if (event.data === "#") {
        this.hashtagKeyPressed = true;
      } else {
        this.hashtagKeyPressed = false;
      }
    },
    sendTypingRequest() {
      this.$emit("show-typing", this.roomId);
    },
    resizeTextarea() {
    ...
    editMessage(message) {
      ...
      usertags.forEach(index => {
        ...
        const user = this.room.users.find(user => user.id === userId);
